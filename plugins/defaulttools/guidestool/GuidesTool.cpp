...
void GuidesTool::mouseMoveEvent(KoPointerEvent *event)
{
    if (m_mode == EditGuide && ! m_isMoving) {
        ...
        if (line.second < 0) {
            useCursor(Qt::ArrowCursor);
            setStatusText(QObject::tr("Double click to add guide line."));
        } else {
            useCursor(line.first == Qt::Horizontal ? Qt::SizeVerCursor : Qt::SizeHorCursor);
            setStatusText(QObject::tr("Click and drag to move guide line. Double click to remove guide line."));
        }
    } else {
        setStatusText("");
        repaintDecorations();
...
QList<QWidget*> GuidesTool::createOptionWidgets()
{
    ...
    m_options->setWindowTitle(QObject::tr("Guides Editor"));
    ...
    m_insert->setWindowTitle(QObject::tr("Guides Insertor"));
