    ...
    , m_resetPaint(true)
{
	...
	connectPixmap.load( QCoreApplication::applicationDirPath()+"/data"+"/calligra/icons/cursor_connect.png" );
    m_connectCursor = QCursor(connectPixmap, 4, 1);

    m_editConnectionPoint = new QAction(QObject::tr("Edit connection points"), this);
    ...
    m_alignLeft = new QAction(koIcon("align-horizontal-left"), QObject::tr("Align to left edge"), this);
    m_alignLeft->setCheckable(true);
    addAction("align-left", m_alignLeft);
    m_alignCenterH = new QAction(koIcon("align-horizontal-center"), QObject::tr("Align to horizontal center"), this);
    m_alignCenterH->setCheckable(true);
    addAction("align-centerh", m_alignCenterH);
    m_alignRight = new QAction(koIcon("align-horizontal-right"), QObject::tr("Align to right edge"), this);
    m_alignRight->setCheckable(true);
    addAction("align-right", m_alignRight);
    m_alignTop = new QAction(koIcon("align-vertical-top"), QObject::tr("Align to top edge"), this);
    m_alignTop->setCheckable(true);
    addAction("align-top", m_alignTop);
    m_alignCenterV = new QAction(koIcon("align-vertical-center"), QObject::tr("Align to vertical center"), this);
    m_alignCenterV->setCheckable(true);
    addAction("align-centerv", m_alignCenterV);
    m_alignBottom = new QAction(koIcon("align-vertical-bottom"), QObject::tr("Align to bottom edge"), this);
    m_alignBottom->setCheckable(true);
    addAction("align-bottom", m_alignBottom);

    m_escapeAll = new QAction(koIcon("escape-direction-all"), QObject::tr("Escape in all directions"), this);
    m_escapeAll->setCheckable(true);
    addAction("escape-all", m_escapeAll);
    m_escapeHorizontal = new QAction(koIcon("escape-direction-horizontal"), QObject::tr("Escape in horizontal directions"), this);
    m_escapeHorizontal->setCheckable(true);
    addAction("escape-horizontal", m_escapeHorizontal);
    m_escapeVertical = new QAction(koIcon("escape-direction-vertical"), QObject::tr("Escape in vertical directions"), this);
    m_escapeVertical->setCheckable(true);
    addAction("escape-vertical", m_escapeVertical);
    m_escapeLeft = new QAction(koIcon("escape-direction-left"), QObject::tr("Escape in left direction"), this);
    m_escapeLeft->setCheckable(true);
    addAction("escape-left", m_escapeLeft);
    m_escapeRight = new QAction(koIcon("escape-direction-right"), QObject::tr("Escape in right direction"), this);
    m_escapeRight->setCheckable(true);
    addAction("escape-right", m_escapeRight);
    m_escapeUp = new QAction(koIcon("escape-direction-up"), QObject::tr("Escape in up direction"), this);
    m_escapeUp->setCheckable(true);
    addAction("escape-up", m_escapeUp);
    m_escapeDown = new QAction(koIcon("escape-direction-down"), QObject::tr("Escape in down direction"), this);
    ...
    switch(m_editMode) {
        case Idle:
            if (m_currentShape) {
                if (dynamic_cast<KoConnectionShape*>(m_currentShape)) {
                    if (m_activeHandle >= 0)
                        emit statusTextChanged(QObject::tr("Drag to edit connection."));
                    else
                        emit statusTextChanged(QObject::tr("Double click connection or press delete to remove it."));
                } else if (m_activeHandle < 0) {
                    emit statusTextChanged(QObject::tr("Click to edit connection points."));
                }
            } else {
        ...
        case EditConnectionPoint:
            if (m_activeHandle >= KoConnectionPoint::FirstCustomConnectionPoint)
                emit statusTextChanged(QObject::tr("Drag to move connection point. Double click connection or press delete to remove it."));
            else if (m_activeHandle >= 0)
                emit statusTextChanged(QObject::tr("Double click connection point or press delete to remove it."));
            else
                emit statusTextChanged(QObject::tr("Double click to add connection point."));
            break;
        case CreateConnection:
            emit statusTextChanged(QObject::tr("Drag to create new connection."));
            break;
        default:
            emit statusTextChanged("");
    }
...
QList<QWidget *> ConnectionTool::createOptionWidgets()
{
    ...
    if (factory) {
        ...
        foreach(KoShapeConfigWidgetBase *cw, widgets) {
            ...
            cw->setWindowTitle(QObject::tr("Connection"));
            list.append(cw);
        }
    }
    ConnectionPointWidget *connectPoint = new ConnectionPointWidget(this);
    connectPoint->setWindowTitle(QObject::tr("Connection Point"));
