...
StencilBoxDocker::StencilBoxDocker(QWidget* parent)
    : QDockWidget(parent)
{
    setWindowTitle(QObject::tr("Stencil Box"));
    QWidget* mainWidget = new QWidget(this);
    setWidget(mainWidget);

    m_menu = new QMenu();
    QAction *ghnsAction = m_menu->addAction(koIcon("get-hot-new-stuff"), QObject::tr("Get more stencils"));
    QAction *installAction = m_menu->addAction(koIcon("document-open-folder"), QObject::tr("Install stencil"));
    ...
    m_button->setToolTip(QObject::tr("More shapes"));
...
void StencilBoxDocker::getHotNewStuff()
{
    ...
    //    QMessageBox::information(0, QObject::tr("Stencils successfully installed."));
    //}
    //else if(!dialog.changedEntries().isEmpty()) {
    //    QMessageBox::information(0, QObject::tr("Stencils successfully uninstalled."));
    //}
}
...
void StencilBoxDocker::installStencil()
{
    ...
    //       + QObject::tr("Calligra Stencil Packages (*.cstencil.tar, *.cstencil.tar.bz2, *.cstencil.tar.gz)")
    //       , this);
	QString path = QFileDialog::getOpenFileName(this
		, QObject::tr("Open File")
		, dir.path()
        , "Calligra Stencil Packages (*.cstencil.tar, *.cstencil.tar.bz2, *.cstencil.tar.gz)");
    if(path.isNull()) return;
    
    KoTar archive(path);
    if(!archive.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(0, "", QObject::tr("Could not read this package."));
        return;
    }
    
    //QString destination = KStandardDirs::locate("data", "flow/stencils");
	QString destination = QCoreApplication::applicationDirPath()+"/data"+"/flow/stencils";
    const KoArchiveDirectory* const archiveDir = archive.directory();
    ...
    if(QFile::exists(collectionFolder)) {
        QMessageBox::critical(0, "", QObject::tr("A collection of stencils in family '%1' is already installed. "
...
void StencilBoxDocker::loadDefaultShapes()
{
    ...
    foreach( const QString & id, KoShapeRegistry::instance()->keys() )
    {
        ...
        foreach( const KoShapeTemplate & shapeTemplate, factory->templates() )
        {
            ...
            temp.icon = QIcon(shapeTemplate.iconName);
        ...
        if(!oneAdded)
        {
            ...
            temp.icon = QIcon(factory->iconName());
            temp.properties = 0;
...
void StencilBoxDocker::loadShapeCollections()
{
	...
	QStringList dirs;
	dirs += QCoreApplication::applicationDirPath()+"/data/flow/stencils";
	//dirs += QCoreApplication::applicationDirPath()+"/data/flow/stencils/Circuit2";
...
bool StencilBoxDocker::addCollection(const QString& path)
{
	...
	QSettings dg(dir.absoluteFilePath("collection.desktop"), QSettings::IniFormat); 
	dg.beginGroup("Desktop Entry");
    QString family = dg.value("Name").toString();
    QString type = dg.value("X-KDE-DirType").toString();
	dg.endGroup();
    kDebug() << family << type;
    ...
    foreach(const QString & stencil, stencils) {
		...
		QSettings content(dir.absoluteFilePath(stencil), QSettings::IniFormat); 
		...
		content.beginGroup("Desktop Entry");
        QString keepAspectRatio = content.value("CS-KeepAspectRatio", "0").toString();
		content.endGroup();
