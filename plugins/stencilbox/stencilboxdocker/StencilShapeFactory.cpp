...
KoShape *StencilShapeFactory::createDefaultShape(KoDocumentResourceManager *documentResources) const
{
    ...
    //if ( !arr.isEmpty() ) {
        ...
        if(store->bad())
        {
            //emit loadingFailed(QObject::tr("Not a valid Calligra file: %1", m_path));
            delete store;
            return shape;
        }
        ...
        if (body.isNull()) {
            ...
            //emit loadingFailed(QObject::tr("No office:drawing tag found in file: %1", m_path));
            delete store;
            return shape;
        }
        ...
        if (page.isNull()) {
            ...
            //emit loadingFailed(QObject::tr("No page found in file: %1", m_path));
            delete store;
            return shape;
        }
        ...
        if (group.isNull()) {
            ...
            //emit loadingFailed(QObject::tr("No group found in file: %1", m_path));
            delete store;
            return shape;
        }
        ...
        if (n_shape.isNull()) {
            ...
            //emit loadingFailed(QObject::tr("No shapes found in file: %1", m_path));
            delete store;
            return shape;
        }
