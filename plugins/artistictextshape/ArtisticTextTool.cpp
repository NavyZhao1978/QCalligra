...
{
    m_detachPath  = new QAction(koIcon("artistictext-detach-path"), QObject::tr("Detach Path"), this);
    ...
    m_convertText  = new QAction(koIcon("pathshape"), QObject::tr("Convert to Path"), this);
    ...
    m_fontBold = new QAction(koIcon("format-text-bold"), QObject::tr("Bold text"), this);
    ...
    m_fontItalic = new QAction(koIcon("format-text-italic"), QObject::tr("Italic text"), this);
    ...
    m_superScript = new QAction(koIcon("format-text-superscript"), QObject::tr("Superscript"), this);
    ...
    m_subScript = new QAction(koIcon("format-text-subscript"), QObject::tr("Subscript"), this);
    ...
    QAction *anchorStart = new QAction(koIcon("format-justify-left"), QObject::tr("Anchor at Start"), this);
    ...
    QAction *anchorMiddle = new QAction(koIcon("format-justify-center"), QObject::tr("Anchor at Middle"), this);
    ...
    QAction *anchorEnd = new QAction(koIcon("format-justify-right"), QObject::tr("Anchor at End"), this);
	...
	connect(edit_select_all, SIGNAL(triggered()), this, SLOT(selectAll()));
	edit_select_all->setShortcut(QKeySequence(QKeySequence::SelectAll));
    addAction("edit_select_all", edit_select_all);
	...
	connect(edit_deselect_all, SIGNAL(triggered()), this, SLOT(deselectAll()));
	//edit_deselect_all->setShortcut(QKeySequence(QKeySequence::DeselectAll));
    addAction("edit_deselect_all", edit_deselect_all);
...
void ArtisticTextTool::mouseMoveEvent( KoPointerEvent *event )
{
    ...
    if ( m_hoverText ) {
        ...
        if (m_hoverText == m_currentShape)
            emit statusTextChanged(QObject::tr("Click to change cursor position."));
        else
            emit statusTextChanged(QObject::tr("Click to select text shape."));
    } else if( m_hoverPath && m_currentShape && !hoverOnBaseline) {
        useCursor( QCursor( Qt::PointingHandCursor ) );
        emit statusTextChanged(QObject::tr("Double click to put text on path."));
    } else  if (m_hoverHandle) {
        useCursor( QCursor( Qt::ArrowCursor ) );
        emit statusTextChanged( QObject::tr("Drag handle to change start offset.") );
    } else {
        useCursor( QCursor( Qt::ArrowCursor ) );
        if (m_currentShape)
            emit statusTextChanged( QObject::tr("Press escape to finish editing.") );
        else
...
void ArtisticTextTool::activate(ToolActivation toolActivation, const QSet<KoShape*> &shapes)
{
    ...
    updateActions();
    emit statusTextChanged( QObject::tr("Press return to finish editing.") );
    repaintDecorations();
...
void ArtisticTextTool::convertText()
{
    ...
    cmd->setText( QObject::tr("Convert to Path") );
...
QList<QWidget *> ArtisticTextTool::createOptionWidgets()
{
    ...
    configWidget->setWindowTitle(QObject::tr("Text Properties"));
    ...
    pathWidget->setWindowTitle(QObject::tr("Text On Path"));
