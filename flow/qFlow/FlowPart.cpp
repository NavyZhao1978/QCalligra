...
void FlowPart::showStartUpWidget(KoMainWindow *parent, bool alwaysShow)
{
    ...
    //if (!factory) {
    //    m_errorMessage = QObject::tr("Can not find needed text component, Calligra Flow will quit now.");
    //    error = true;
    //}
    factory = KoShapeRegistry::instance()->value("PictureShape");
    if (!factory) {
        m_errorMessage = QObject::tr("Can not find needed picture component, Calligra Flow will quit now.");
        error = true;
    }
...
{
    QMessageBox::critical(0, m_errorMessage, QObject::tr("Installation Error"));
