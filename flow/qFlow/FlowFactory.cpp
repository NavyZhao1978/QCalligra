...
QString* FlowFactory::s_instance = 0;
//KAboutData* FlowFactory::s_aboutData = 0;

FlowFactory::FlowFactory(QObject* parent)
  : KoPluginFactory(parent)
//  : KoPluginFactory(*aboutData(), parent)
{
  (void) componentData();
}
...
QString FlowFactory::id() const
{
	return QString(componentData());
 }

const QString &FlowFactory::componentData()
{
  if (!s_instance) {
    s_instance = new QString("Flow");
