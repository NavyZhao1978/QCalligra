    ...
    , d(new Private(this))
{
    ...
    setWindowTitle( QObject::tr("Import Data" ) );

    QStringList encodings;
    encodings << QObject::tr( "Descriptive encoding name, Recommended ( %1 )").arg("UTF-8" );
    encodings << QObject::tr( "Descriptive encoding name, Locale ( %1 )").arg(QString(QTextCodec::codecForLocale()->name() ));
    encodings += KGlobal::charsets()->descriptiveEncodingNames();
    // Add a few non-standard encodings, which might be useful for text files
    const QString description(QObject::tr("Descriptive encoding name, Other ( %1 )"));
...
void KoCsvImportDialog::setDataTypes(DataTypes dataTypes)
{
    ...
    if (dataTypes & Generic)
        d->formatList << QObject::tr("Generic");
    if (dataTypes & Text)
        d->formatList << QObject::tr("Text");
    if (dataTypes & Date)
        d->formatList << QObject::tr("Date");
    if (dataTypes & Currency)
        d->formatList << QObject::tr("Currency");
    if (dataTypes & None)
        d->formatList << QObject::tr("None");
...
void KoCsvImportDialog::Private::loadSettings()
{
	...
	configGroup.beginGroup("CSVDialog Settings");
...
void KoCsvImportDialog::Private::fillTable()
{
    ...
    for (column = 0; column < dialog->m_sheet->columnCount(); ++column)
    {
        ...
        if (!headerItem || !formatList.contains(headerItem->text())) {
            dialog->m_sheet->setHorizontalHeaderItem(column, new QTableWidgetItem(QObject::tr("Generic")));
        }
    }
...
KoCsvImportDialog::DataType KoCsvImportDialog::dataType(int col) const
{
    ...
    if (header == QObject::tr("Generic"))
        return Generic;
    else if (header == QObject::tr("Text"))
        return Text;
    else if (header == QObject::tr("Date"))
        return Date;
    else if (header == QObject::tr("Currency"))
        return Currency;
    else if (header == QObject::tr("None"))
        return None;
    return Generic;
}
...
bool KoCsvImportDialog::Private::checkUpdateRange()
{
    ...
    {
        QMessageBox::critical(0, QObject::tr("Please check the ranges you specified. The start value must be lower than the end value."));
        return false;
    }
    return true;
}
...
QTextCodec* KoCsvImportDialog::Private::updateCodec() const
{
	...
	QString strCodec = dialog->comboBoxEncoding->currentText();
    bool ok = false;
    ...
    if ( !codec || !ok )
    {
        ...
        QMessageBox::critical( 0, QObject::tr("Cannot find encoding: %1", strCodec ) );
        return 0;
    }
