...
KoResourceTagging::KoResourceTagging(const QString& extensions)
{
	...
	m_tagsXMLFile = QCoreApplication::applicationDirPath()+"/data"+"/krita/tags.xml";
    //m_config = KConfigGroup( KGlobal::config(), "resource tagging" );
	QSettings m_config("QCalligra", QCoreApplication::applicationName());
	m_config.beginGroup("resource tagging" );
    m_nepomukOn = m_config.value("nepomuk_usage_for_resource_tagging", false).toBool();
	m_config.endGroup();
...
void KoResourceTagging::updateTagRepoFromNepomuk(bool serverIdentity)
{
    ...
    foreach(Nepomuk::Resource resource, resourceList) {
        ...
        QString resourceFileName = correctedNepomukFileName(resource.toFile().toString().path());
