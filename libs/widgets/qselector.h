...
/**
 * QSelector is the base class for other widgets which
 * provides the ability to choose from a one-dimensional
 * range of values. An example is the QGradientSelector
 * which allows to choose from a range of colors.
 *
 * A custom drawing routine for the widget surface has
 * to be provided by the subclass.
 */
class KOWIDGETS_EXPORT QSelector : public QAbstractSlider
{
  Q_OBJECT
...
public:
   ...
   */
  explicit QSelector( QWidget *parent=0 );
  /**
   * Constructs a one-dimensional selection widget with
   * a given orientation.
   */
  explicit QSelector( Qt::Orientation o, QWidget *parent = 0 );
  /*
...
/**
 * The QGradientSelector widget allows the user to choose
 ...
 *
 **/
class KOWIDGETS_EXPORT QGradientSelector : public QSelector
{
  Q_OBJECT
...
public:
   ...
   */
  explicit QGradientSelector( QWidget *parent=0 );
  /**
   * Constructs a colors selector with orientation o which
   * contains a gradient between white and black.
   */
  explicit QGradientSelector( Qt::Orientation o, QWidget *parent=0 );
  /**
   * Destructs the widget.
   */
  ~QGradientSelector();
...
private:
  class QGradientSelectorPrivate;
  friend class QGradientSelectorPrivate;
  QGradientSelectorPrivate * const d;
  
  Q_DISABLE_COPY(QGradientSelector)
};
