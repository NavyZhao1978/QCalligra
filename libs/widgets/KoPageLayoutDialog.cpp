...
KoPageLayoutDialog::KoPageLayoutDialog(QWidget *parent, const KoPageLayout &layout)
    ...
    , d(new Private)
{
    setWindowTitle(QObject::tr("Page Layout"));
    //setFaceType(KoTabDialog::Tabbed);

    QWidget *widget = new QWidget(this);
    addTab(widget, QObject::tr("Page"));
    ...
    if (on && d->documentCheckBox == 0) {
        for (int i = 0; i < children().count(); ++i) {
            if (QDialogButtonBox *buttonBox = qobject_cast<QDialogButtonBox*>(children()[i])) {
                d->documentCheckBox = new QCheckBox(QObject::tr("Apply to document"), buttonBox);
