...
void KoEditColorSetWidget::addColor()
{
    ...
    if ( color.isValid() ) {
        ...
        newEntry.name = QInputDialog::getText(this, QObject::tr("Add Color To Palette"), QObject::tr("Color name:"));
        KoColorPatch *patch = new KoColorPatch(widget.patchesFrame);
...
{
    Q_ASSERT(m_activeColorSet);
    //QString fileName = QFileDialog::getOpenFileName(QUrl("file://"+m_activeColorSet->filename()), "*.gpl", this);
	QString fileName = QFileDialog::getOpenFileName(this, QObject::tr("Open File"), m_activeColorSet->filename(),"*.gpl");
...
void KoEditColorSetWidget::save()
{
    ...
    if (!m_activeColorSet->save())
        QMessageBox::critical(0, QObject::tr("Palette"), QObject::tr("Cannot write to palette file %1. Maybe it is read-only. ").arg(m_activeColorSet->filename()) );
}
    ...
    : QDialog(parent)
{
    ...
    setWindowTitle(QObject::tr("Add/Remove Colors"));
    //enableButton(QDialog::Accepted, ui->isEnabled());
}
