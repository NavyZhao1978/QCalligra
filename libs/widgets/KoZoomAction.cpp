...
void KoZoomAction::regenerateItems(const qreal zoom, bool asCurrent)
{
    ...
    foreach(qreal value, zoomLevels) {
        if(value>10.0)
            //values << QObject::tr("%1%").arg(KGlobal::locale()->formatNumber(value, 0));
			values << QObject::tr("%1%").arg(QString::number( value, 'f', 0 ) );
        else
            //values << QObject::tr("%1%").arg(KGlobal::locale()->formatNumber(value, 1));
			values << QObject::tr("%1%").arg(QString::number( value, 'f', 1 ) );
    }
    ...
    if(asCurrent)
    {
        QString valueString;
        if(zoom*100>10.0)
            //valueString = QObject::tr("%1%").arg(KGlobal::locale()->formatNumber(zoom*100, 0));
			valueString = QObject::tr("%1%").arg(QString::number( zoom*100, 'f', 0 ) );
        else
            //valueString = QObject::tr("%1%").arg(KGlobal::locale()->formatNumber(zoom*100, 1));
			valueString = QObject::tr("%1%").arg(QString::number( zoom*100, 'f', 1 ) );
...
QWidget * KoZoomAction::createWidget(QWidget *parent)
{
    ...
    if (d->specialButtons & ZoomToSelection) {
        ...
        zoomToSelectionButton->setToolTip(QObject::tr("Zoom to Selection"));
    ...
    if (d->specialButtons & ZoomToAll) {
        ...
        zoomToAllButton->setToolTip(QObject::tr("Zoom to All"));
