...
void KoColorSetWidgetPrivate::fillColors()
{
    ...
    if (colorSet) {
        for( int i = 0, p= 0; i < colorSet->nColors(); i++) {
            ...
            thePublic->connect(patch, SIGNAL(triggered(KoColorPatch *)), thePublic, SLOT(colorTriggered(KoColorPatch *)));
...
void KoColorSetWidgetPrivate::addRemoveColors()
{
    ...
    if (dlg->exec() == QDialog::Accepted ) { // always reload the color set
...
void KoColorSetWidgetPrivate::addRecent(const KoColor &color)
{
    if(numRecents<6) {
        ...
        thePublic->connect(recentPatches[numRecents], SIGNAL(triggered(KoColorPatch *)), thePublic, SLOT(colorTriggered(KoColorPatch *)));
        numRecents++;
    }
...
void KoColorSetWidgetPrivate::activateRecent(int i)
{
    ...
    ,d(new KoColorSetWidgetPrivate())
{
    ...
    d->recentsLayout->addWidget(new QLabel(QObject::tr("Recent:")));
    ...
    d->addRemoveButton->setText(QObject::tr("Add / Remove Colors..."));
...
void KoColorSetWidgetPrivate::colorTriggered(KoColorPatch *patch)
{
    int i;
