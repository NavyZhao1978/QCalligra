...
void KoSliderComboPrivate::showPopup()
{
...
void KoSliderComboPrivate::hidePopup()
{
    container->hide();
}
...
void KoSliderComboPrivate::lineEditFinished()
{
...
void KoSliderComboPrivate::sliderValueChanged(int slidervalue)
{
    //thePublic->setEditText(KGlobal::locale()->formatNumber(minimum + (maximum - minimum)*slidervalue/256, decimals));
	thePublic->setEditText(QString::number(minimum + (maximum - minimum)*slidervalue/256,  'f', decimals));

    qreal value = thePublic->currentText().toDouble();
    emit thePublic->valueChanged(value, false);
}

void KoSliderComboPrivate::sliderReleased()
{
    qreal value = thePublic->currentText().toDouble();
    emit thePublic->valueChanged(value, true);
}
...
void KoSliderCombo::setValue(qreal value)
{
	...
	setEditText(QString::number(value, 'f', d->decimals));
