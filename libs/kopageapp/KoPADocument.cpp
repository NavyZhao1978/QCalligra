    ...
    d(new Private())
{
    ...
    //connect(documentInfo(), SIGNAL(infoUpdated(const QString &, const QString &)), d->inlineTextObjectManager, SLOT(documentInformationUpdated(const QString &, const QString &)));
...
KoPAPageProvider *KoPADocument::getPageProvider()
{
	return d->pageProvider;
}
...
bool KoPADocument::loadOdf( KoOdfReadStore & odfStore)
{
    ...
    KoOdfLoadingContext loadingContext( odfStore.styles(), odfStore.store(), QCoreApplication::applicationName());
...
void KoPADocument::loadConfig()
{
    //if( config->hasGroup( "Grid" ) )
    {
		...
		QSettings configGroup("QCalligra", QCoreApplication::applicationName());
		configGroup.beginGroup("Grid");
        bool showGrid = configGroup.value( "ShowGrid", defGrid.showGrid() ).toBool();
        gridData().setShowGrid(showGrid);
        bool paintGridInBackground = configGroup.value("PaintGridInBackground", defGrid.paintGridInBackground()).toBool();
        gridData().setPaintGridInBackground(paintGridInBackground);
        bool snapToGrid = configGroup.value( "SnapToGrid", defGrid.snapToGrid() ).toBool();
        gridData().setSnapToGrid(snapToGrid);
        qreal spacingX = configGroup.value( "SpacingX", defGrid.gridX() ).toDouble();
        qreal spacingY = configGroup.value( "SpacingY", defGrid.gridY() ).toDouble();
        gridData().setGrid( spacingX, spacingY );
        QColor color = configGroup.value( "Color", defGrid.gridColor().value() ).toInt();
        gridData().setGridColor( color );
		configGroup.endGroup();
    ...
    //if( config->hasGroup( "Interface" ) )
    {
		...
		QSettings configGroup("QCalligra", QCoreApplication::applicationName());
		configGroup.beginGroup("Interface");
        bool showRulers = configGroup.value( "ShowRulers", true).toBool();
...
void KoPADocument::saveConfig()
{
	QSettings configGroup("QCalligra", QCoreApplication::applicationName());
	{
		...
		configGroup.setValue("ShowGrid", showGrid);

		bool snapToGrid = gridData().snapToGrid();
		configGroup.setValue("SnapToGrid", snapToGrid);

		qreal spacingX = gridData().gridX();
		configGroup.setValue("SpacingX", spacingX);

		qreal spacingY = gridData().gridY();
		configGroup.setValue("SpacingY", spacingY);

		QColor color = gridData().gridColor();
		configGroup.setValue("Color", color.value());
...

	{
		...
		configGroup.setValue("ShowRulers", showRulers);
		configGroup.endGroup();
	}
}
