...
{
    return isSlideType ?
        (isMaster ? QObject::tr("Master Slide %1/%2").arg(pageNumber).arg(pageCount)
                  : QObject::tr("Slide %1/%2").arg(pageNumber).arg(pageCount)) :
        (isMaster ? QObject::tr("Master Page %1/%2").arg(pageNumber).arg(pageCount)
                  : QObject::tr("Page %1/%2").arg(pageNumber).arg(pageCount));
}
...
{
    //QMenu *actionCollection = d->view->actionCollection();
