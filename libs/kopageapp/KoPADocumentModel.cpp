            ...
            if( name.isEmpty() )
            {
                if ( dynamic_cast<KoPAPageBase *>( shape ) ) {
                    if(m_document->pageType() == KoPageApp::Slide ) {
                        name = QObject::tr("Slide %1").arg(m_document->pageIndex(dynamic_cast<KoPAPageBase *>(shape)) + 1);
                    } else {
                        name = QObject::tr("Page  %1").arg(m_document->pageIndex(dynamic_cast<KoPAPageBase *>(shape)) + 1);
                    }
                }
                else if( dynamic_cast<KoShapeLayer*>( shape ) ) {
                    name = QObject::tr("Layer") + QString(" (%1)").arg(shape->zIndex());
                }
                else if( dynamic_cast<KoShapeGroup*>( shape ) ) {
                    name = QObject::tr("Group") + QString(" (%1)").arg(shape->zIndex());
                }
                else {
                    name = QObject::tr("Shape") + QString(" (%1)").arg(shape->zIndex());
                }
            }
            return name;
        }
...
bool KoPADocumentModel::setData(const QModelIndex &index, const QVariant &value, int role )
{
    ...
    switch (role)
    {
        ...
        case Qt::EditRole:
        {
            ...
            if (dynamic_cast<KoPAPageBase *>(shape)) {
                if (m_document->pageType() == KoPageApp::Slide) {
                    cmd->setText(QObject::tr("Rename Slide"));
                }
                else {
                    cmd->setText(QObject::tr("Rename Page"));
                }
            }
            else if (dynamic_cast<KoShapeLayer *>(shape)) {
                cmd->setText(QObject::tr("Rename Layer"));
            }
...
KoDocumentSectionModel::PropertyList KoPADocumentModel::properties( KoShape* shape ) const
{
    ...
    if (KoPAPageBase *page = dynamic_cast<KoPAPageBase *>(shape)) {
        ...
        l << Property(QObject::tr("Slide"), QString::number(m_document->pageIndex(page) + 1));
    }

    l << Property(QObject::tr("Visible"), koIcon("layer-visible-on"), koIcon("layer-visible-off"), shape->isVisible());
    l << Property(QObject::tr("Locked"), koIcon("object-locked"), koIcon("object-unlocked"), shape->isGeometryProtected());
    return l;
}
...
bool KoPADocumentModel::dropMimeData( const QMimeData * data, Qt::DropAction action, int row, int column, const QModelIndex & parent )
{
    ...
    if( container ) {
        ...
        if( group )
        {
            ...
            cmd->setText( QObject::tr("Reparent shapes") );
...
bool KoPADocumentModel::doDrop(QList<KoPAPageBase *> pages, KoPAPageBase *pageAfter, Qt::DropAction action)
{
    ...
    if (((modifiers & Qt::ControlModifier) == 0) &&
        ((modifiers & Qt::ShiftModifier) == 0)) {
           ...
           seq.chop(1);
           QAction *popupMoveAction = new QAction(QObject::tr("&Move Here") + '\t' + seq, this);
           popupMoveAction->setIcon(koIcon("go-jump"));
           seq = QKeySequence(Qt::ControlModifier).toString();
           seq.chop(1);
           QAction *popupCopyAction = new QAction(QObject::tr("&Copy Here") + '\t' + seq, this);
           popupCopyAction->setIcon(koIcon("edit-copy"));
           seq = QKeySequence( Qt::ControlModifier + Qt::ShiftModifier ).toString();
           seq.chop(1);
           QAction *popupCancelAction = new QAction(QObject::tr("C&ancel") + '\t' + QKeySequence(Qt::Key_Escape).toString(), this);
