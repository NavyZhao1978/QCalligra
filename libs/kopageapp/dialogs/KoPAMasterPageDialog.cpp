    ...
    m_document(document)
{
    ...
    if(m_document->pageType() == KoPageApp::Slide ) {
        layout->addWidget(new QLabel(QObject::tr("Select a master slide design:"), mainWidget));
        setWindowTitle(QObject::tr("Master Slide"));
    } else {
        layout->addWidget(new QLabel(QObject::tr("Select a master page design:"), mainWidget));
        setWindowTitle(QObject::tr("Master Page"));
    }
