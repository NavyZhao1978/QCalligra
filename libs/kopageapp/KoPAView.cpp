...
public:
    ...
    QAction *actionViewShowMasterPages;
...
void KoPAView::initGUI()
{
	...
	QMenu *ctrlMenu = mainWindow()->menuBar()->addMenu(QObject::tr("Controller"));
	KoCanvasControllerWidget *canvasController = new KoCanvasControllerWidget( ctrlMenu, this );
	...
	QMenu *toolsMenu = mainWindow()->menuBar()->addMenu(QObject::tr("Tools"));
	KoToolManager::instance()->registerTools( toolsMenu, d->canvasController );
	
    //d->zoomController = new KoZoomController( d->canvasController, zoomHandler(), actionCollection());	
	QMenu *zoomMenu = mainWindow()->menuBar()->addMenu(QObject::tr("Zoom"));
	d->zoomController = new KoZoomController( d->canvasController, zoomHandler(), zoomMenu);
...
void KoPAView::initActions()
{
	...
	QAction *edit_cut = new QAction(QObject::tr("edit_cut"), this);
	//connect(edit_cut, SIGNAL(triggered()), this, SLOT(edit_cut()));
	edit_cut->setShortcut(QKeySequence(QKeySequence::Cut));
    //actionCollection()->addAction("edit_cut", edit_cut);
	editMenu->addAction(edit_cut);
    d->cutController = new KoCutController(kopaCanvas(), edit_cut);

    //action = actionCollection()->addAction( KStandardAction::Copy, "edit_copy", 0, 0 );
	QAction *edit_copy = new QAction(QObject::tr("edit_copy"), this);
	//connect(edit_copy, SIGNAL(triggered()), this, SLOT(edit_copy()));
	edit_copy->setShortcut(QKeySequence(QKeySequence::Copy));
    //actionCollection()->addAction("edit_copy", edit_copy);
	editMenu->addAction(edit_copy);
    d->copyController = new KoCopyController(kopaCanvas(), edit_copy);

    //d->editPaste = actionCollection()->addAction( KStandardAction::Paste, "edit_paste", proxyObject, SLOT( editPaste() ) );	
	QAction *edit_paste = new QAction(QObject::tr("edit_paste"), this);
	connect(edit_paste, SIGNAL(triggered()), proxyObject, SLOT(editPaste()));
	edit_paste->setShortcut(QKeySequence(QKeySequence::Paste));
    //actionCollection()->addAction("edit_paste", edit_paste);
	editMenu->addAction(edit_paste);
	d->editPaste = edit_paste;
	...
	QAction *edit_select_all = new QAction(QObject::tr("edit_select_all"), this);
	connect(edit_select_all, SIGNAL(triggered()), this, SLOT(editSelectAll()));
	edit_select_all->setShortcut(QKeySequence(QKeySequence::SelectAll));
    //actionCollection()->addAction("edit_select_all", edit_select_all);
	editMenu->addAction(edit_select_all);

    //actionCollection()->addAction(KStandardAction::Deselect,  "edit_deselect_all", this, SLOT(editDeselectAll()));
	QAction *edit_deselect_all = new QAction(QObject::tr("edit_deselect_all"), this);
	connect(edit_deselect_all, SIGNAL(triggered()), this, SLOT(editDeselectAll()));
	//edit_deselect_all->setShortcut(QKeySequence(QKeySequence::DeselectAll));
    //actionCollection()->addAction("edit_deselect_all", edit_deselect_all);
	editMenu->addAction(edit_deselect_all);

    d->deleteSelectionAction = new QAction(koIcon("edit-delete"), QObject::tr("D&elete"), this);
    //actionCollection()->addAction("edit_delete", d->deleteSelectionAction );
	editMenu->addAction(d->deleteSelectionAction);
    ...
    QAction *showGrid= d->doc->gridData().gridToggleAction(d->canvas);
    //actionCollection()->addAction("view_grid", showGrid );
	editMenu->addAction(showGrid);

    d->actionViewSnapToGrid = new QAction(QObject::tr("Snap to Grid"), this);
	...
	editMenu->addAction(d->actionViewSnapToGrid);
    connect( d->actionViewSnapToGrid, SIGNAL( triggered( bool ) ), this, SLOT (viewSnapToGrid( bool )));

    QAction *actionViewShowGuides = KoStandardAction::showGuides(this, SLOT(viewGuides(bool)), this);
	...
	editMenu->addAction(actionViewShowGuides);

	QMenu *pageMenu = mainWindow()->menuBar()->addMenu(QObject::tr("&Page"));
    d->actionViewShowMasterPages = new QAction(QObject::tr("Show Master Pages" ), this );
    //actionCollection()->addAction( "view_masterpages", d->actionViewShowMasterPages );
	pageMenu->addAction(d->actionViewShowMasterPages);
    connect( d->actionViewShowMasterPages, SIGNAL( triggered( bool ) ), this, SLOT( setMasterMode( bool ) ) );

    d->viewRulers  = new QAction(QObject::tr("Show Rulers"), this);
    //actionCollection()->addAction("view_rulers", d->viewRulers );
	pageMenu->addAction(d->viewRulers);
    d->viewRulers->setToolTip(QObject::tr("Show/hide the view's rulers"));
    connect(d->viewRulers, SIGNAL(triggered(bool)), proxyObject, SLOT(setShowRulers(bool)));
    setShowRulers(d->doc->rulersVisible());

    d->actionInsertPage = new QAction(koIcon("document-new"), QObject::tr("Insert Page"), this);
    //actionCollection()->addAction( "page_insertpage", d->actionInsertPage );
	pageMenu->addAction(d->actionInsertPage);
    d->actionInsertPage->setToolTip( QObject::tr("Insert a new page after the current one" ) );
    d->actionInsertPage->setWhatsThis( QObject::tr("Insert a new page after the current one" ) );
    connect( d->actionInsertPage, SIGNAL( triggered() ), proxyObject, SLOT( insertPage() ) );

    d->actionCopyPage = new QAction( QObject::tr("Copy Page" ), this );
    //actionCollection()->addAction( "page_copypage", d->actionCopyPage );
	pageMenu->addAction(d->actionCopyPage);
    d->actionCopyPage->setToolTip( QObject::tr("Copy the current page" ) );
    d->actionCopyPage->setWhatsThis( QObject::tr("Copy the current page" ) );
    connect( d->actionCopyPage, SIGNAL( triggered() ), this, SLOT( copyPage() ) );

    d->actionDeletePage = new QAction( QObject::tr("Delete Page" ), this );
    d->actionDeletePage->setEnabled( d->doc->pageCount() > 1 );
    //actionCollection()->addAction( "page_deletepage", d->actionDeletePage );
	pageMenu->addAction(d->actionDeletePage);
    d->actionDeletePage->setToolTip( QObject::tr("Delete the current page" ) );
    d->actionDeletePage->setWhatsThis( QObject::tr("Delete the current page" ) );
    connect( d->actionDeletePage, SIGNAL( triggered() ), this, SLOT( deletePage() ) );

    d->actionMasterPage = new QAction(QObject::tr("Master Page..."), this);
    //actionCollection()->addAction("format_masterpage", d->actionMasterPage);
	pageMenu->addAction(d->actionMasterPage);
    connect(d->actionMasterPage, SIGNAL(triggered()), this, SLOT(formatMasterPage()));

    d->actionPageLayout = new QAction( QObject::tr("Page Layout..." ), this );
    //actionCollection()->addAction( "format_pagelayout", d->actionPageLayout );
	pageMenu->addAction(d->actionPageLayout);
	...
	QAction *page_previous = new QAction(QObject::tr("page_previous"), this);
	connect(page_previous, SIGNAL(triggered()), this, SLOT(goToPreviousPage()));
    //actionCollection()->addAction("page_previous", page_previous);
	pageMenu->addAction(page_previous);

    //actionCollection()->addAction(KStandardAction::Next,  "page_next", this, SLOT(goToNextPage()));
	QAction *page_next = new QAction(QObject::tr("page_next"), this);
	connect(page_next, SIGNAL(triggered()), this, SLOT(goToNextPage()));
    //actionCollection()->addAction("page_next", page_next);
	pageMenu->addAction(page_next);

    //actionCollection()->addAction(KStandardAction::FirstPage,  "page_first", this, SLOT(goToFirstPage()));
	QAction *page_first = new QAction(QObject::tr("page_first"), this);
	connect(page_first, SIGNAL(triggered()), this, SLOT(goToFirstPage()));
    //actionCollection()->addAction("page_first", page_first);
	pageMenu->addAction(page_first);

    //actionCollection()->addAction(KStandardAction::LastPage,  "page_last", this, SLOT(goToLastPage()));
	QAction *page_last = new QAction(QObject::tr("page_last"), this);
	connect(page_last, SIGNAL(triggered()), this, SLOT(goToLastPage()));
    //actionCollection()->addAction("page_first", page_last);
	pageMenu->addAction(page_last);

    d->pageNavigator->initActions();

    //KActionMenu *actionMenu = new KActionMenu(QObject::tr("Variable"), this);
	...
	QMenu *cfgMenu = mainWindow()->menuBar()->addMenu(QObject::tr("&Configure"));
    QAction * am = new QAction(QObject::tr("Import Document..."), this);
    //actionCollection()->addAction("import_document", am);
	cfgMenu->addAction(am);
    connect(am, SIGNAL(triggered()), this, SLOT(importDocument()));

	QAction *exportSvg  = new QAction(QObject::tr("Export as SVG..."), this);
    exportSvg->setIcon(QIcon("application-svg"));
    //actionCollection()->addAction("file_export_svg", exportSvg);
	cfgMenu->addAction(exportSvg);

	QAction *importSvg  = new QAction(QObject::tr("Import from SVG..."), this);
    importSvg->setIcon(QIcon("application-svg"));
    //actionCollection()->addAction("file_import_svg", importSvg);
	cfgMenu->addAction(importSvg);

    d->actionConfigure = new QAction(koIcon("configure"), QObject::tr("Configure..."), this);
    //actionCollection()->addAction("configure", d->actionConfigure);
	cfgMenu->addAction(d->actionConfigure);
...
void KoPAView::importDocument()
{
    ...
    if ( d->doc->pageType() == KoPageApp::Slide ) {
        dialog->setWindowTitle(QObject::tr("Import Slideshow"));
    }
    else {
...
//#endif
	...
	mimeFilter += QString::fromLocal8Bit("application/vnd.oasis.opendocument.graphics-template");

    dialog->setNameFilters( mimeFilter );
    if (dialog->exec() == QDialog::Accepted) {
        QUrl url(dialog->getOpenFileName());
        QString tmpFile = dialog->getOpenFileName();
        //if ( KIO::NetAccess::download( url, tmpFile, 0 ) ) {
            ...
            if ( ! paste.paste( d->doc->documentType(), &data ) ) {
                QMessageBox::critical(0, "", QObject::tr("Could not import\n%1").arg(url.toString()));
            }
        //}
        //else {
        //    QMessageBox::critical(0, "", QObject::tr("Could not import\n%1").arg(url.toString()));
...
void KoPAView::formatPageLayout()
{
    ...
    if ( dialog.exec() == QDialog::Accepted ) {
        KUndo2Command *command = new KUndo2Command( QObject::tr("Change page layout" ) );
                                    ...
                                    const char * format, int quality )
{
    ...
    if ( !pix.isNull() ) {
        ...
        //}
        const bool bLocalFile = fileUrl.isValid();
