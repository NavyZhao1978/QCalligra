...
bool KoPAPastePage::process( const KoXmlElement & body, KoOdfReadStore & odfStore )
{
    ...
    if ( m_doc->pageType() == KoPageApp::Slide ) {
        cmd = new KUndo2Command( QObject::tr("Paste Slides %1").arg(qMax( newMasterPages.size(), newPages.size() ) ) );
    }
    else {
        cmd = new KUndo2Command( QObject::tr("Paste Pages %1").arg(qMax( newMasterPages.size(), newPages.size() ) ) );
    }
