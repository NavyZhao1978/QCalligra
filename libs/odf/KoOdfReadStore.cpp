...
{
    if (!d->store) {
        errorMessage = QObject::tr("No store backend");
        return false;
    }
    ...
    if (!d->store->isOpen()) {
        if (!d->store->open(fileName)) {
            ...
            errorMessage = QObject::tr("Could not find %1").arg(fileName);
            return false;
        }
    }
...
bool KoOdfReadStore::loadAndParse(QIODevice *fileDevice, KoXmlDocument &doc, QString &errorMessage, const QString &fileName)
{
    ...
    if (!ok) {
        ...
        errorMessage = QObject::tr("Parsing error in the main document at line %1, column %2\nError message: %3")
                           .arg(errorLine)
						   .arg(errorColumn)
						   .arg(errorMsg);
    } else {
