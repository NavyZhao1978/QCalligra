...
KoTarStore::KoTarStore(const QString & _filename, Mode _mode, const QByteArray & appIdentification)
{
    ...
    m_pTar = new KoTar(_filename, "application/x-gzip");
...
KoTarStore::KoTarStore(QWidget* window, const QUrl& _url, const QString & _filename, Mode _mode, const QByteArray & appIdentification)
{
    kDebug(30002) << "KoTarStore Constructor url=" << _url.toString()
    << " filename = " << _filename
    << " mode = " << int(_mode) << endl;
    Q_D(KoStore);
    ...
    m_pTar = new KoTar(d->localFileName, "application/x-gzip");
...
// When reading, d->stream comes directly from KoArchiveFile::device()
...
{
    Q_D(KoStore);
    const KoArchiveEntry * entry = m_pTar->directory()->entry(name);
    if (entry == 0) {
    ...
    KoArchiveFile * f = (KoArchiveFile *) entry;
    m_byteArray.resize(0);
    delete d->stream;
...
bool KoTarStore::enterAbsoluteDirectory(const QString& path)
{
    ...
    if (d->mode == Read) {
        m_currentDir = dynamic_cast<const KoArchiveDirectory*>(m_pTar->directory()->entry(path));
