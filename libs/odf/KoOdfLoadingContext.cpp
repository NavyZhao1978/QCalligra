...
KoOdfLoadingContext::KoOdfLoadingContext(KoOdfStylesReader &stylesReader, KoStore* store, const QString &componentData)
        : d(new Private(stylesReader, store))
{
    ...
    if (!componentData.isEmpty()) {
		...
		QString fileName = QCoreApplication::applicationDirPath()+"/flow/styles"+"/defaultstyles.xml";
        if ( ! fileName.isEmpty() ) {
            QFile file( fileName );
