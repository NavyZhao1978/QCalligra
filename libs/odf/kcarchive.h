...
class KoArchiveDirectory;
class KoArchiveFile;

class KoArchivePrivate;
/**
 * KoArchive is a base class for reading and writing archives.
 * @short generic class for reading/writing archives
 * @author David Faure <faure@kde.org>
 */
class KOODF_EXPORT KoArchive
{
protected:
    /**
     ...
     */
    KoArchive( const QString& fileName );
...
public:
     ...
     */
    const KoArchiveDirectory* directory() const;
     ...
     * using this function. KoArchive won't write one directory twice.
     *
...
protected:
     ...
     */
    virtual KoArchiveDirectory* rootDir();
     ...
     */
    KoArchiveDirectory * findOrCreate( const QString & path );
     ...
     * Note that KoArchive will -not- own the device, it must be deleted by the derived class.
     */
     ...
     */
    void setRootDir( KoArchiveDirectory *rootDir );

protected:
    virtual void virtual_hook( int id, void* data );
private:
    KoArchivePrivate* const d;
};

class KoArchiveEntryPrivate;
/**
 * A base class for entries in an KoArchive.
 * @short Base class for the archive-file's directory structure.
 *
 * @see KoArchiveFile
 * @see KoArchiveDirectory
 */
class KOODF_EXPORT KoArchiveEntry
{
public:
    /**
     * Creates a new entry.
     ...
     */
    KoArchiveEntry( KoArchive* archive, const QString& name, int access, int date,
                   const QString& user, const QString& group,
                   const QString& symlink );

    virtual ~KoArchiveEntry();
...
protected:
    KoArchive* archive() const;

protected:
    virtual void virtual_hook( int id, void* data );
private:
    KoArchiveEntryPrivate* const d;
};

class KoArchiveFilePrivate;
/**
 * Represents a file entry in a KoArchive.
 * @short A file in an archive.
 *
 * @see KoArchive
 * @see KoArchiveDirectory
 */
class KOODF_EXPORT KoArchiveFile : public KoArchiveEntry
{
public:
    /**
     * Creates a new file entry. Do not call this, KoArchive takes care of it.
     * @param archive the entries archive
     * @param name the name of the entry
     * @param access the permissions in unix format
     * @param date the date (in seconds since 1970)
     ...
     * @param size the size of the file
     */
    KoArchiveFile( KoArchive* archive, const QString& name, int access, int date,
                  const QString& user, const QString& group, const QString &symlink,
                  qint64 pos, qint64 size );

    /**
     * Destructor. Do not call this, KoArchive takes care of it.
     */
    virtual ~KoArchiveFile();
...
protected:
    virtual void virtual_hook( int id, void* data );
private:
    KoArchiveFilePrivate* const d;
};

class KoArchiveDirectoryPrivate;
/**
 * Represents a directory entry in a KoArchive.
 * @short A directory in an archive.
 *
 * @see KoArchive
 * @see KoArchiveFile
 */
class KOODF_EXPORT KoArchiveDirectory : public KoArchiveEntry
{
public:
    /**
     ...
     * @param symlink the symlink, or QString()
     */
    KoArchiveDirectory( KoArchive* archive, const QString& name, int access, int date,
                   const QString& user, const QString& group,
                   const QString& symlink);

    virtual ~KoArchiveDirectory();
     ...
     */
    const KoArchiveEntry* entry( const QString& name ) const;

    /**
     * @internal
     * Adds a new entry to the directory.
     */
    void addEntry( KoArchiveEntry* );
...
protected:
    virtual void virtual_hook( int id, void* data );
private:
    KoArchiveDirectoryPrivate* const d;
};
