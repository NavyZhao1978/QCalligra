...
public:
    KoZipPrivate()
        : m_crc( 0 ),
          ...
          m_currentDev( 0 ),
          m_compression( 8 ),
          m_extraField( KoZip::NoExtraField ),
	  m_offset( 0 )
    {}
    ...
    QList<KoZipFileEntry*> m_fileList;    // flat list of all files, for the index (saves a recursive method ;)
    int                     m_compression;
    KoZip::ExtraField        m_extraField;
...
KoZip::KoZip( const QString& fileName )
    : KoArchive( fileName ),d(new KoZipPrivate)
{
}

KoZip::KoZip( QIODevice * dev )
    : KoArchive( dev ),d(new KoZipPrivate)
{
}
...
bool KoZip::openArchive( QIODevice::OpenMode mode )
{
    //kDebug(7040);
    d->m_fileList.clear();
    ...
    // KoArchive::open() opened the underlying device already.
...
/*                // here we calculate the length of the file in the zip
	    ...
	    if (os_madeby == 3) {	// good ole unix
            ...
            if ( isdir )
            {
                ...
                {
                    entry = new KoArchiveDirectory( this, entryName, access, (int)pfi.mtime, rootDir()->user(), rootDir()->group(), QString() );
                    //kDebug(7040) << "KoArchiveDirectory created, entryName= " << entryName << ", name=" << name;
                }
	    }
            else
            {
		...
		if (S_ISLNK(access)) {
		    symlink = QFile::decodeName(pfi.guessed_symlink);
                ...
                entry = new KoZipFileEntry( this, entryName, access, pfi.mtime,
					rootDir()->user(), rootDir()->group(),
					symlink, name, dataoffset,
					ucsize, cmethod, csize );
                static_cast<KoZipFileEntry *>(entry)->setHeaderStart( localheaderoffset );
                static_cast<KoZipFileEntry*>(entry)->setCRC32(crc32);
                //kDebug(7040) << "KoZipFileEntry created, entryName= " << entryName << ", name=" << name;
            ...
            if ( entry )
            {
                ...
                else
                {
                    ...
                    KoArchiveDirectory * tdir = findOrCreate( path );
                    tdir->addEntry(entry);
                }
            }
...
bool KoZip::closeArchive()
{
    ...
    QMutableListIterator<KoZipFileEntry*> it( d->m_fileList );
...
bool KoZip::doWriteDir( const QString &name, const QString &user, const QString &group,
...
bool KoZip::doPrepareWriting(const QString &name, const QString &user,
    ...
    KoArchiveDirectory* parentDir = rootDir();
    ...
    KoZipFileEntry * e = new KoZipFileEntry( this, fileName, perm, mtime, user, group, QString(),
    ...
    d->m_currentDev = KoFilterDev::device( device(), QString::fromLatin1("application/x-gzip"), false );
...
bool KoZip::doFinishWriting( qint64 size )
{
...
bool KoZip::doWriteSymLink(const QString &name, const QString &target,
...
{
    KoArchive::virtual_hook( id, data );
}

bool KoZip::writeData(const char * data, qint64 size)
{
    Q_ASSERT( d->m_currentFile );
    Q_ASSERT( d->m_currentDev );
...
void KoZip::setExtraField( ExtraField ef )
{
    d->m_extraField = ef;
}

KoZip::ExtraField KoZip::extraField() const
{
...
////////////////////// KoZipFileEntry////////////////////////////////////
////////////////////////////////////////////////////////////////////////
class KoZipFileEntry::KoZipFileEntryPrivate
{
public:
    KoZipFileEntryPrivate()
    : crc(0),
      compressedSize(0),
      headerStart(0),
      encoding(0)
...
KoZipFileEntry::KoZipFileEntry(KoZip* zip, const QString& name, int access, int date,
                             const QString& user, const QString& group, const QString& symlink,
                             const QString& path, qint64 start, qint64 uncompressedSize,
                             int encoding, qint64 compressedSize)
 : KoArchiveFile(zip, name, access, date, user, group, symlink, start, uncompressedSize ),
   d(new KoZipFileEntryPrivate)
{
    d->path = path;
    d->encoding = encoding;
...
KoZipFileEntry::~KoZipFileEntry()
{
    delete d;
}

int KoZipFileEntry::encoding() const
{
    return d->encoding;
}

qint64 KoZipFileEntry::compressedSize() const
{
    return d->compressedSize;
}

void KoZipFileEntry::setCompressedSize(qint64 compressedSize)
{
    d->compressedSize = compressedSize;
}

void KoZipFileEntry::setHeaderStart(qint64 headerstart)
{
    d->headerStart = headerstart;
}

qint64 KoZipFileEntry::headerStart() const
{
    return d->headerStart;
}

unsigned long KoZipFileEntry::crc32() const
{
    return d->crc;
}

void KoZipFileEntry::setCRC32(unsigned long crc32)
{
    d->crc=crc32;
}

const QString &KoZipFileEntry::path() const
{
    return d->path;
}
...
QIODevice* KoZipFileEntry::createDevice() const
{
    ...
    if ( encoding() == 8 )
    {
        ...
        QIODevice* filterDev = KoFilterDev::device( limitedDev, QString::fromLatin1("application/x-gzip") );
        if ( !filterDev )
            return 0L; // ouch
