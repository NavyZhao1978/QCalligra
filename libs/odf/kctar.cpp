...
class KoTar::KoTarPrivate
{
public:
    KoTarPrivate(KoTar *parent)
      : q(parent),
        tarEnd( 0 ),
        tmpFile( 0 )
    {
...
KoTar::KoTar( const QString& fileName, const QString & _mimetype )
    : KoArchive( fileName ), d(new KoTarPrivate(this))
{
    d->mimetype = _mimetype;
}

KoTar::KoTar( QIODevice * dev )
    : KoArchive( dev ), d(new KoTarPrivate(this))
{
    Q_ASSERT( dev );
}

// Only called when a filename was given
bool KoTar::createDevice(QIODevice::OpenMode mode)
{
    ...
    if (d->mimetype == QLatin1String("application/x-tar")) {
        return KoArchive::createDevice(mode);
    } else if (mode == QIODevice::WriteOnly) {
        if (!KoArchive::createDevice(mode))
            return false;
        if (!d->mimetype.isEmpty()) {
            // Create a compression filter on top of the KSaveFile device that KoArchive created.
            //kDebug(7041) << "creating KoFilterDev for" << d->mimetype;
            QIODevice *filterDev = KoFilterDev::device(device(), d->mimetype);
            Q_ASSERT(filterDev);
            setDevice(filterDev);
        }
        return true;
...
KoTar::~KoTar()
{
    // mjarrett: Closes to prevent ~KoArchive from aborting w/o device
    if( isOpen() )
        close();
...
void KoTar::setOrigFileName( const QByteArray & fileName ) {
    if ( !isOpen() || !(mode() & QIODevice::WriteOnly) )
    {
        kWarning(7041) << "KoTar::setOrigFileName: File must be opened for writing first.\n";
        return;
    }
    d->origFileName = fileName;
}

qint64 KoTar::KoTarPrivate::readRawHeader( char *buffer ) {
  // Read header
  ...
  if ( n == 0x200 && buffer[0] != 0 ) {
    ...
    if (strncmp(buffer + 257, "ustar", 5)) {
      ...
      if( strncmp( buffer + 148 + 6 - s.length(), s.data(), s.length() )
        ...
        kWarning(7041) << "KoTar: invalid TAR file. Header is:" << QByteArray( buffer+257, 5 )
...
bool KoTar::KoTarPrivate::readLonglink(char *buffer,QByteArray &longlink) {
  qint64 n = 0;
...
qint64 KoTar::KoTarPrivate::readHeader( char *buffer, QString &name, QString &symlink ) {
  name.truncate(0);
  symlink.truncate(0);
  while (true) {
 ...
 */
bool KoTar::KoTarPrivate::fillTempFile( const QString & fileName) {
    if ( ! tmpFile )
        return true;
    ...
    QIODevice *filterDev = KoFilterDev::deviceForFile( fileName, mimetype, forced );
...
bool KoTar::openArchive( QIODevice::OpenMode mode ) {
    ...
    do
    {
        ...
        if (n == 0x200)
        {
            ...
            if ( isdir )
            {
                ...
                e = new KoArchiveDirectory( this, nm, access, time, user, group, symlink );
            }
            else
            {
                // read size
                ...
                else
                {
                    ...
                    e = new KoArchiveFile( this, nm, access, time, user, group, symlink,
                                          dev->pos(), size );
                }
            ...
            if ( pos == -1 )
            {
                if (nm == QLatin1String(".")) { // special case
                    Q_ASSERT( isdir );
                    if ( isdir )
                        setRootDir( static_cast<KoArchiveDirectory *>( e ) );
                }
                else
                    rootDir()->addEntry( e );
            }
            else
            {
                ...
                KoArchiveDirectory * d = findOrCreate( path );
                d->addEntry( e );
            }
        }
        else
 ...
 */
bool KoTar::KoTarPrivate::writeBackTempFile( const QString & fileName )
{
    if ( !tmpFile )
        return true;
    ...
    // (KoArchive uses KSaveFile by default, but the temp-uncompressed-file trick
    // circumvents that).

    QIODevice *dev = KoFilterDev::deviceForFile( fileName, mimetype, forced );
    if( dev ) {
        QFile* file = tmpFile;
        ...
        if ( forced )
            static_cast<KoFilterDev *>(dev)->setOrigFileName( origFileName );
        file->seek(0);
        QByteArray buffer;
        buffer.resize(8*1024);
        qint64 len;
...
bool KoTar::doFinishWriting( qint64 size ) {
    // Write alignment
    int rest = size % 0x200;
...
void KoTar::KoTarPrivate::fillBuffer( char * buffer,
...
void KoTar::KoTarPrivate::writeLonglink(char *buffer, const QByteArray &name, char typeflag,
                                      const char *uname, const char *gname) {
...
bool KoTar::doPrepareWriting(const QString &name, const QString &user,
    ...
    /*
      ...
      // But as KoTar and the "tar" program both handle tar files without
...
bool KoTar::doWriteDir(const QString &name, const QString &user,
...
bool KoTar::doWriteSymLink(const QString &name, const QString &target,
...
void KoTar::virtual_hook( int id, void* data ) {
