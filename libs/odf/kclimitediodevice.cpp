...
KoLimitedIODevice::KoLimitedIODevice( QIODevice *dev, qint64 start, qint64 length )
...
void KoLimitedIODevice::close()
{
}

qint64 KoLimitedIODevice::size() const
{
    return m_length;
}

qint64 KoLimitedIODevice::readData( char * data, qint64 maxlen )
{
    maxlen = qMin( maxlen, m_length - pos() ); // Apply upper limit
    return m_dev->read( data, maxlen );
}

bool KoLimitedIODevice::seek( qint64 pos )
{
...
qint64 KoLimitedIODevice::bytesAvailable() const
{
     return m_length + QIODevice::bytesAvailable();
}

bool KoLimitedIODevice::isSequential() const
{
