...
KoStore* KoStore::createStore(QWidget* window, const QUrl& url, Mode mode, const QByteArray & appIdentification, Backend backend)
{
    ...
    if (mode == KoStore::Write) {
        ...
        const bool downloaded = false; //KIO::NetAccess::download(url, tmpFile, window);
        if (!downloaded) {
...
//#endif
    default:
        ...
        QMessageBox::warning(window,
						   QObject::tr("Calligra Storage"),
                           QObject::tr("The directory mode is not supported for remote locations."));
        return 0;
    }
}
...
bool KoStorePrivate::extractFile(const QString &srcName, QIODevice &buffer)
{
    ...
    // ### This could use KoArchive::copy or something, no?
