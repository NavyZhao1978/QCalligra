...
void KoGzipFilter::init(int mode)
{
    init(mode, filterFlags() == WithHeaders ? GZipHeader : RawDeflate);
}

void KoGzipFilter::init(int mode, Flag flag)
{
    if (d->isInitialized) {
        terminate();
    }
    ...
    } else if ( mode == QIODevice::WriteOnly )
    {
        ...
        qWarning() << "KoGzipFilter: Unsupported mode " << mode << ". Only QIODevice::ReadOnly and QIODevice::WriteOnly supported";
    }
    d->mode = mode;
    d->compressed = true;
...
int KoGzipFilter::mode() const
{
    return d->mode;
}

void KoGzipFilter::terminate()
{
...
bool KoGzipFilter::writeHeader( const QByteArray & fileName )
{
...
void KoGzipFilter::setOutBuffer( char * data, uint maxlen )
{
    d->zStream.avail_out = maxlen;
    d->zStream.next_out = (Bytef *) data;
}
void KoGzipFilter::setInBuffer( const char * data, uint size )
{
#ifdef DEBUG_GZIP
...
int KoGzipFilter::inBufferAvailable() const
{
    return d->zStream.avail_in;
}
int KoGzipFilter::outBufferAvailable() const
{
    return d->zStream.avail_out;
}

KoGzipFilter::Result KoGzipFilter::uncompress_noop()
{
    ...
    if ( d->zStream.avail_in > 0 )
    {
        ...
        return KoFilterBase::Ok;
    } else
        return KoFilterBase::End;
}

KoGzipFilter::Result KoGzipFilter::uncompress()
{
#ifndef NDEBUG
    if (d->mode == 0) {
        qWarning() << "mode==0; KoGzipFilter::init was not called!";
        return KoFilterBase::Error;
    } else if (d->mode == QIODevice::WriteOnly) {
...
#endif
        return ( result == Z_OK ? KoFilterBase::Ok : ( result == Z_STREAM_END ? KoFilterBase::End : KoFilterBase::Error ) );
    } else
        return uncompress_noop();
}

KoGzipFilter::Result KoGzipFilter::compress( bool finish )
{
    ...
    KoGzipFilter::Result callerResult = result == Z_OK ? KoFilterBase::Ok : (Z_STREAM_END ? KoFilterBase::End : KoFilterBase::Error);
