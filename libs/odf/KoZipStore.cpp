...
{
    kDebug(30002) << "KoZipStore Constructor url" << _url.toString()
    << " filename = " << _filename
    << " mode = " << int(_mode)
    << " mimetype = " << appIdentification << endl;
    Q_D(KoStore);
...
{
    Q_D(KoStore);
    const KoArchiveEntry * entry = m_pZip->directory()->entry(name);
    if (entry == 0) {
    ...
    // Must cast to KoZipFileEntry, not only KoArchiveFile, because device() isn't virtual!
