...
KoEncryptedStore::KoEncryptedStore(QWidget* window, const QUrl& url, const QString & filename, Mode mode, const QByteArray & appIdentification)
        : m_qcaInit(QCA::Initializer()), m_password(QCA::SecureArray()), m_filename(QString(url.toString())), m_manifestBuffer(QByteArray()), m_tempFile(NULL), m_bPasswordUsed(false), m_bPasswordDeclined(false), m_currentDir(NULL)
{
    Q_D(KoStore);
    ...
    if (mode == Read) {
        ...
        d->localFileName = filename;
        m_pZip = new KoZip(d->localFileName);
    } else {
...
bool KoEncryptedStore::init(Mode mode, const QByteArray & appIdentification)
{
    ...
    if (mode == Write) {
        ...
        //    m_pZip->setExtraField(KoZip::NoExtraField);
        ...
        const KoArchiveEntry* manifestArchiveEntry = m_pZip->directory()->entry(MANIFEST_FILE);
        if (!manifestArchiveEntry || !manifestArchiveEntry->isFile()) {
            // No manifest file? OK, *I* won't complain
            return true;
        }
        QIODevice *dev = (static_cast< const KoArchiveFile* >(manifestArchiveEntry))->createDevice();
        ...
        if (!xmldoc.setContent(dev, namespaceProcessing) || xmldoc.documentElement().localName() != "manifest" || xmldoc.documentElement().namespaceURI() != KoXmlNS::manifest) {
            QMessageBox::warning(0, "", QObject::tr("The manifest file seems to be corrupted. The document could not be opened."));
            dev->close();
            delete dev;
            m_pZip->close();
            d->good = false;
            ...
            while (!xmlnode.isNull()) {
                    ...
                    if (xmlencnode.toElement().hasAttribute("checksum-type")) {
                        ...
                        // For this particual hash-type: check KoEncryptedStore_encryptionData.checksumShort
                            ...
                            if (!checksumErrorShown) {
                                QMessageBox::warning(0, "", QObject::tr("This document contains an unknown checksum. When you give a password it might not be verified."));
                    ...
                    if (xmlencattr.toElement().localName() == "algorithm" && xmlencattr.toElement().hasAttribute("initialisation-vector")) {
                        ...
                        if (xmlencattr.toElement().hasAttribute("algorithm-name") && xmlencattr.toElement().attribute("algorithm-name") != "Blowfish CFB") {
                            if (!unreadableErrorShown) {
                                QMessageBox::warning(0, "", QObject::tr("This document contains an unknown encryption method. Some parts may be unreadable."));
                ...
                if (!(encData.salt.isEmpty() || encData.initVector.isEmpty())) {
                    ...
                    if (!(algorithmFound && keyDerivationFound)) {
                        if (!unreadableErrorShown) {
                            QMessageBox::warning(0, "", QObject::tr("This document contains incomplete encryption data. Some parts may be unreadable."));
                            unreadableErrorShown = true;
                        }
                    }
                }
        ...
        if (isEncrypted() && !(QCA::isSupported("sha1") && QCA::isSupported("pbkdf2(sha1)") && QCA::isSupported("blowfish-cfb"))) {
            d->good = false;
            QMessageBox::critical(0, "", QObject::tr("QCA has currently no support for SHA1 or PBKDF2 using SHA1. The document can not be opened."));
        }
    }
    return d->good;
}
    ...
    if (d->good) {
        ...
        if (d->mode == Write) {
            ...
            if (!m_manifestBuffer.isEmpty() && !document.setContent(m_manifestBuffer)) {
                ...
                QMessageBox::critical(0, "", QObject::tr("The manifest file seems to be corrupted. It cannot be modified and the document will remain unreadable. Please try and save the document again to prevent losing your work."));
                m_pZip->close();
                return false;
            }
            ...
            if (!m_pZip->writeFile(MANIFEST_FILE, "", "", m_manifestBuffer.data(), m_manifestBuffer.size())) {
                QMessageBox::critical(0, "", QObject::tr("The manifest file cannot be written. The document will remain unreadable. Please try and save the document again to prevent losing your work."));
                m_pZip->close();
                return false;
            }
        }
...
bool KoEncryptedStore::openRead(const QString& name)
{
    ...
    const KoArchiveEntry* fileArchiveEntry = m_pZip->directory()->entry(name);
    if (!fileArchiveEntry) {
        return false;
    }
    ...
    const KoZipFileEntry* fileZipEntry = static_cast<const KoZipFileEntry*>(fileArchiveEntry);
    ...
    if (m_encryptionData.contains(name)) {
        ...
        while (true) {
            ...
            if (!m_password.isEmpty()) {
                ...
                //dlg.setPrompt(QObject::tr("Please enter the password to open this file."));
                //if (! dlg.exec()) {
        ...
        QIODevice *resultDevice = KoFilterDev::device(new QBuffer(resultArray, NULL), "application/x-gzip");
        if (!resultDevice) {
            delete resultArray;
            return false;
        }
        static_cast<KoFilterDev*>(resultDevice)->setSkipHeaders();
...
bool KoEncryptedStore::closeWrite()
{
    ...
    //    dlg.setPrompt(QObject::tr("Please enter the password to encrypt the document with."));
    //    if (! dlg.exec()) {
    ...
    if (passWasAsked && QMessageBox::question(d->window, "", QObject::tr("Do you want to save the password?"), QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
        savePasswordInKWallet();
    }
    ...
    if (d->fileName == THUMBNAIL_FILE) {
        ...
        QIODevice *compressDevice = KoFilterDev::device(&compressedData, "application/x-gzip", false);
        if (!compressDevice) {
            return false;
        }
        static_cast<KoFilterDev*>(compressDevice)->setSkipHeaders();
...
bool KoEncryptedStore::enterRelativeDirectory(const QString& dirName)
{
    ...
    if (d->mode == Read) {
        ...
        const KoArchiveEntry *entry = m_currentDir->entry(dirName);
        if (entry && entry->isDirectory()) {
            m_currentDir = dynamic_cast<const KoArchiveDirectory*>(entry);
...
bool KoEncryptedStore::enterAbsoluteDirectory(const QString& path)
{
    ...
    m_currentDir = dynamic_cast<const KoArchiveDirectory*>(m_pZip->directory()->entry(path));
    return m_currentDir != 0;
}

bool KoEncryptedStore::fileExists(const QString& absPath) const
{
    const KoArchiveEntry *entry = m_pZip->directory()->entry(absPath);
