...
{
    KUndo2Action *result = new KUndo2Action(QObject::tr("Undo %1"), QObject::tr("Undo"), parent);
...
{
    KUndo2Action *result = new KUndo2Action(QObject::tr("Redo %1"), QObject::tr("Redo"), parent);
...
QAction* KUndo2Stack::createRedoAction(QActionGroup* actionCollection, const QString& actionName)
{
    QAction* action = KUndo2QStack::createRedoAction(actionCollection);

    if(actionName.isEmpty()) {
        action->setObjectName(QKeySequence(QKeySequence::Redo).toString());
    } else {
        action->setObjectName(actionName);
    }

    action->setIcon(koIcon("edit-redo"));
    action->setIconText(QObject::tr("Redo"));
...
QAction* KUndo2Stack::createUndoAction(QActionGroup* actionCollection, const QString& actionName)
{
    QAction* action = KUndo2QStack::createUndoAction(actionCollection);

    if(actionName.isEmpty()) {
        action->setObjectName(QKeySequence(QKeySequence::Undo).toString());
    } else {
        action->setObjectName(actionName);
    }

    action->setIcon(koIcon("edit-undo"));
    action->setIconText(QObject::tr("Undo"));
