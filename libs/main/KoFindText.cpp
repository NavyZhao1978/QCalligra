    ...
    : KoFindBase(parent), d(new Private(this))
{
    ...
    options->addOption("caseSensitive", QObject::tr("Case Sensitive"), QObject::tr("Match cases when searching"), QVariant::fromValue<bool>(false));
    options->addOption("wholeWords", QObject::tr("Whole Words Only"), QObject::tr("Match only whole words"), QVariant::fromValue<bool>(false));
    options->addOption("fromCursor", QObject::tr("Find from Cursor"), QObject::tr("Start searching from the current cursor"), QVariant::fromValue<bool>(true));
    setOptions(options);
}
