    ...
    QDialog(parent)
{
    ...
    if (filenames.size() == 1) {
        layout->addWidget(new QLabel(QObject::tr("The following autosave file can be recovered:")));
    }
    else {
        layout->addWidget(new QLabel(QObject::tr("The following autosave files can be recovered:")));
    }

    m_listView = new QListView();
    m_listView->setAcceptDrops(false);
    QStyledItemDelegate *delegate = new FileItemDelegate(m_listView, this);
