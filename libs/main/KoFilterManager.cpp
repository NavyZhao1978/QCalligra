...
KoFilterManager::KoFilterManager(KoDocument* document,
        ...
        d(new KoFilterManagerPrivate(progressUpdater))
{
    d->batch = false;
}
...
KoFilterManager::KoFilterManager(const QString& url, const QByteArray& mimetypeHint,
        ...
        m_graph(""), d(new KoFilterManagerPrivate)
{
    d->batch = false;
}

KoFilterManager::KoFilterManager(const QByteArray& mimeType) :
        m_document(0), m_parentChain(0), m_graph(""), d(new KoFilterManagerPrivate)
{
    d->batch = false;
...
KoFilter::ConversionStatus KoFilterManager::exportDocument(const QString& url, QByteArray& mimeType)
{
    ...
    if (!m_graph.isValid()) {
        kError(30500) << "Couldn't create a valid graph for this source mimetype.";
        if (!d->batch && !userCancelled) QMessageBox::critical(0, QObject::tr("Could not export file."), QObject::tr("Missing Export Filter"));
    ...
    if (!chain) {
        ...
        if (!d->batch) QMessageBox::critical(0, QObject::tr("Could not export file."), QObject::tr("Missing Export Filter"));
        return KoFilter::BadConversionGraph;
    }
...
void buildGraph(QHash<QByteArray, Vertex*>& vertices, KoFilterManager::Direction direction)
{
    ...
    for (; it != end; ++it) {
        ...
        if (direction == KoFilterManager::Import) {
            ...
            foreach(const QString & testIt, (*it)->import_) {
                if (!stopList.contains(testIt))
...
{
    QString tmp = QObject::tr("Could not import file of type\n%1").arg(mimeType);
    // ###### FIXME: use KLibLoader::lastErrorMessage() here
    if (!suppressDialog) QMessageBox::critical(0, tmp, QObject::tr("Missing Import Filter"));
}
