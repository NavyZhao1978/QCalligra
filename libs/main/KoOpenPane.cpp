...
KoOpenPane::KoOpenPane(QWidget *parent, const QString &componentData, const QStringList& mimeFilter, const QString& templateType)
        : QWidget(parent)
...
{
    QString header = QObject::tr("Recent Documents");
...
void KoOpenPane::addCustomDocumentWidget(QWidget *widget, const QString& title, const QString& icon)
{
    ...
    if (realtitle.isEmpty())
        realtitle = QObject::tr("Custom Document");
...
QTreeWidgetItem* KoOpenPane::addPane(const QString &title, const QString &iconName, QWidget *widget, int sortWeight)
{
    ...
    listItem->setIcon(0, QIcon(iconName));
...
void KoOpenPane::initExistingFilesPane( const QStringList& mimeFilter )
{
    ...
    connect(widget, SIGNAL(openExistingUrl(const QUrl&)),
            this, SIGNAL(openExistingFile(const QUrl&)));
    QTreeWidgetItem* item = addPane(QObject::tr("Open Document"), "document-open", widget, 2);

    //KConfigGroup cfgGrp(d->m_componentData.config(), "TemplateChooserDialog");

    //if (cfgGrp.readEntry("LastReturnType") == QObject::tr("Open Document")) {
        //d->m_sectionList->setCurrentItem(item, 0, QItemSelectionModel::ClearAndSelect);
        //KoSectionListItem* selectedItem = static_cast<KoSectionListItem*>(item);
        //d->m_widgetStack->widget(selectedItem->widgetIndex())->setFocus();
    //}
