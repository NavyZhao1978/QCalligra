...
void KoPart::showSavingErrorDialog()
{
    if (d->document->errorMessage().isEmpty()) {
        QMessageBox::critical(0, "", QObject::tr("Could not save\n%1").arg(localFilePath()));
    } else if (d->document->errorMessage() != "USER_CANCELED") {
        QMessageBox::critical(0, "", QObject::tr("Could not save %1\nReason: %2").arg(localFilePath(), d->document->errorMessage()));
    }
}
...
{
    if (d->document->errorMessage().isEmpty()) {
        QMessageBox::critical(0, "", QObject::tr("Could not open\n%1").arg(localFilePath()));
    } else if (d->document->errorMessage() != "USER_CANCELED") {
        QMessageBox::critical(0, "", QObject::tr("Could not open %1\nReason: %2").arg(localFilePath()).arg(d->document->errorMessage()));
    }
}
...
void KoPart::openExistingFile(const QUrl& url)
{
    ...
    d->document->openUrl(url.toLocalFile());
    setModified(false);
...
void KoPart::openTemplate(const QUrl& url)
{
    ...
    if (ok) {
		...
		QString mimeType = QString::fromLocal8Bit("application/vnd.oasis.opendocument.graphics-template");
...
void KoPart::addRecentURLToAllShells(QUrl url)
{
...
void KoPart::showStartUpWidget(KoMainWindow *mainWindow, bool alwaysShow)
{
    ...
    //                templateURL.setPath(QUrl(desktopfile).path() + '/' + f.readUrl());
    ...
    if (d->startUpWidget) {
        ...
        d->startUpWidget = createOpenPane(mainWindow, QCoreApplication::applicationName(), d->templateType);
...
KoOpenPane *KoPart::createOpenPane(QWidget *parent, const QString &componentData,
