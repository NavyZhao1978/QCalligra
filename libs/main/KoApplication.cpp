...
//#endif
		...
		filters << QString(".%1-%2-%3-autosave%4").arg(QCoreApplication::applicationName()).arg("*").arg("*").arg(extension);
        QDir dir = QDir::home();
        ...
        QStringList pids;
        QString ourPid;
        ourPid.setNum(QCoreApplication::applicationPid());

        //foreach (QString name, reply.value()) {
        //    if (name.contains(QCoreApplication::applicationName())) {
        ...
        const bool print = false;			//koargs->isSet("print");
        const bool exportAsPdf = false;			//koargs->isSet("export-pdf");
        const QString pdfFileName = "filename";			//koargs->getOption("export-filename");
        const QString roundtripFileName = "filename";			//koargs->getOption("roundtrip-filename");
        const bool doTemplate = false;			//koargs->isSet("template");
        const bool benchmarkLoading = false;			//koargs->isSet("benchmark-loading") || koargs->isSet("benchmark-loading-show-window") || !roundtripFileName.isEmpty();
        // only show the shell when no command-line mode option is passed
        const bool showShell = true;			
        ...
        const QString profileFileName = "filename";			//koargs->getOption("profile-filename");
        //koargs->clear();
        ...
        for (int argNumber=1; argNumber<argsCount; argNumber++) {
            // For now create an empty document
            QString errorMsg;
            KoPart *part = entry.createKoPart(&errorMsg);
            if (part) {
                ...
                if (doTemplate) {
                    QStringList paths;
                    if ( !args.at(argNumber).isEmpty() && QFile::exists(args.at(argNumber)) ) {
                        paths << QString(args.at(argNumber));
      ...
      //                  //paths = KGlobal::dirs()->findAllResources("data", appName + "/templates/*/" + desktopName);
						//paths = QDir(QCoreApplication::applicationDirPath()+"/data/flow/templates/*/", "*.desktop").nameFilters();
      //                  if (paths.isEmpty()) {
      //                      //paths = KGlobal::dirs()->findAllResources("data", appName + "/templates/" + desktopName);
						//	paths = QDir(QCoreApplication::applicationDirPath()+"/data/flow/templates/*/", "*.desktop").nameFilters();
      //                  }
      //                  if (paths.isEmpty()) {
      //                      QMessageBox::critical(0, "", QObject::tr("No template found for: %1").arg(desktopName));
      //                      delete shell;
      //                  } else if (paths.count() > 1) {
      //                      QMessageBox::critical(0, "", QObject::tr("Too many templates found for: %1").arg(desktopName));
      //                      delete shell;
      //                  }
                    }
                    ...
                    //    QUrl templateURL;
                    //    templateURL.setPath(templateBase.path() + '/' + templateName);
                    ...
                    //        numberOfOpenDocuments++;
                    //    } else {
                    //        QMessageBox::critical(0, "", QObject::tr("Template %1 failed to load.").arg(templateURL.toString()));
                    //        delete shell;
                    //    }
                    //}
                    // now try to load
                }
                else if (shell->openDocument(part, doc, args.at(argNumber))) {					
                    ...
                    if (benchmarkLoading) {
                        ...
                        if (!roundtripFileName.isEmpty()) {
                            part->saveAs(QUrl("file:"+roundtripFileName));
                        }
                        // close the document
                        shell->slotFileQuit();
