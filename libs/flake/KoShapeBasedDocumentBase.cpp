...
public:
        ...
        : resourceManager(new KoDocumentResourceManager())
    {
        ...
        //if (config->hasGroup("Misc")) 
		{
			...
			miscGroup.beginGroup("Misc");

            qreal pasteOffset = miscGroup.value("CopyOffset", 10.0).toDouble();
            resourceManager->setPasteOffset(pasteOffset);

            bool pasteAtCursor = miscGroup.value("PasteAtCursor", true).toBool();
            resourceManager->enablePasteAtCursor(pasteAtCursor);

            quint32 grabSensitivity = miscGroup.value("GrabSensitivity", 3).toInt();
            resourceManager->setGrabSensitivity(grabSensitivity);

            quint32 handleRadius = miscGroup.value("HandleRadius", 3).toInt();
