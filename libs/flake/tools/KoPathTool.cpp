        ...
        , m_currentStrategy(0)
{
    ...
    m_actionPathPointCorner = new QAction(koIcon("pathpoint-corner"), QObject::tr("Corner point"), this);
    ...
    m_actionPathPointSmooth = new QAction(koIcon("pathpoint-smooth"), QObject::tr("Smooth point"), this);
    ...
    m_actionPathPointSymmetric = new QAction(koIcon("pathpoint-symmetric"), QObject::tr("Symmetric Point"), this);
    ...
    m_actionCurvePoint = new QAction(koIcon("pathpoint-curve"), QObject::tr("Make curve point"), this);
    addAction("pathpoint-curve", m_actionCurvePoint);
    connect(m_actionCurvePoint, SIGNAL(triggered()), this, SLOT(pointToCurve()));

    m_actionLinePoint = new QAction(koIcon("pathpoint-line"), QObject::tr("Make line point"), this);
    addAction("pathpoint-line", m_actionLinePoint);
    connect(m_actionLinePoint, SIGNAL(triggered()), this, SLOT(pointToLine()));

    m_actionLineSegment = new QAction(koIcon("pathsegment-line"), QObject::tr("Segment to Line"), this);
    ...
    m_actionCurveSegment = new QAction(koIcon("pathsegment-curve"), QObject::tr("Segment to Curve"), this);
    ...
    m_actionAddPoint = new QAction(koIcon("pathpoint-insert"), QObject::tr("Insert point"), this);
    ...
    m_actionRemovePoint = new QAction(koIcon("pathpoint-remove"), QObject::tr("Remove point"), this);
    ...
    m_actionBreakPoint = new QAction(koIcon("path-break-point"), QObject::tr("Break at point"), this);
    addAction("path-break-point", m_actionBreakPoint);
    connect(m_actionBreakPoint, SIGNAL(triggered()), this, SLOT(breakAtPoint()));

    m_actionBreakSegment = new QAction(koIcon("path-break-segment"), QObject::tr("Break at segment"), this);
    addAction("path-break-segment", m_actionBreakSegment);
    connect(m_actionBreakSegment, SIGNAL(triggered()), this, SLOT(breakAtSegment()));

    m_actionJoinSegment = new QAction(koIcon("pathpoint-join"), QObject::tr("Join with segment"), this);
    ...
    m_actionMergePoints = new QAction(koIcon("pathpoint-merge"), QObject::tr("Merge points"), this);
    addAction("pathpoint-merge", m_actionMergePoints);
    connect(m_actionMergePoints, SIGNAL(triggered()), this, SLOT(mergePoints()));

    m_actionConvertToPath = new QAction(koIcon("convert-to-path"), QObject::tr("To Path"), this);
...
QList<QWidget *>  KoPathTool::createOptionWidgets()
{
    ...
    updateOptionsWidget();
    toolOptions->setWindowTitle(QObject::tr("Line/Curve"));
    list.append(toolOptions);
    QWidget* widget = d->canvas->createSnapGuideConfigWidget();
    widget->setWindowTitle(QObject::tr("Snapping"));
    list.append(widget);
...
void KoPathTool::mouseMoveEvent(KoPointerEvent *event)
{
    ...
    foreach(KoPathShape *shape, m_pointSelection.selectedShapes()) {
        ...
        if (parameterShape && parameterShape->isParametricShape()) {
            ...
            if (handleId != -1) {
                ...
                emit statusTextChanged(QObject::tr("Drag to move handle."));
                if (m_activeHandle)
                    m_activeHandle->repaint();
                delete m_activeHandle;
            ...
            if (! points.empty()) {
                ...
                if (bestPointType == KoPathPoint::Node)
                    emit statusTextChanged(QObject::tr("Drag to move point. Shift click to change point type."));
                else
                    emit statusTextChanged(QObject::tr("Drag to move control point."));
    ...
    if(hoveredSegment) {
        ...
        emit statusTextChanged(QObject::tr("Drag to change curve directly. Double click to insert new path point."));
        ...
        if (selectedPointCount == 0)
            ...
            emit statusTextChanged(QObject::tr("Press B to break path at selected point."));
        else
            emit statusTextChanged(QObject::tr("Press B to break path at selected segments."));
    }
}
