...
/*!
    \class KoTabDialog
    \compat
    \brief The KoTabDialog class provides a stack of tabbed widgets.
    ...
    KoTabDialog provides a tab bar consisting of single row of tabs at
    the top; each tab has an associated widget which is that tab's
    tab page. In addition, KoTabDialog provides an OK button and the
    following optional buttons: Apply, Cancel, Defaults and Help.

    The normal way to use KoTabDialog is to do the following in the
    constructor:
    \list 1
    \i Create a KoTabDialog.
    ...
    KoTabDialog does not support tabs on the sides or bottom, nor can
    you set or retrieve the visible page. If you need more functionality
    than KoTabDialog provides, consider creating a QDialog and using a
    QTabBar with QTabWidgets.

    Most of the functionality in KoTabDialog is provided by a QTabWidget.
*/

/*!
    \fn void KoTabDialog::selected(const QString &name)
...
/*! \fn void KoTabDialog::currentChanged(QWidget *widget)
...
KoTabDialogPrivate::KoTabDialogPrivate()
	: tw(0),
...
/*!
  Constructs a KoTabDialog with only an OK button.
  The \a parent, \a name, \a modal and widget flag, \a f, arguments
  are passed on to the QDialog constructor.
*/

KoTabDialog::KoTabDialog(QWidget *parent, const char *name, bool modal, Qt::WindowFlags f)
    : QDialog(parent, f)
{
	setWindowTitle(name);
	setModal(modal);

    d = new KoTabDialogPrivate;
    Q_CHECK_PTR(d);

    d->tw = new QTabWidget(this);
	d->tw->setWindowTitle("tab widget");
...
void KoTabDialog::setFont(const QFont & font)
{
...
/*!
  \fn void KoTabDialog::applyButtonPressed();
...
bool KoTabDialog::hasDefaultButton() const
{
     return d->db != 0;
}
...
bool KoTabDialog::hasHelpButton() const
{
     return d->hb != 0;
}
...
/*!
  \fn void KoTabDialog::cancelButtonPressed();
...
bool KoTabDialog::hasCancelButton() const
{
     return d->cb != 0;
}
...
/*!
  \fn void KoTabDialog::defaultButtonPressed();
...
/*!
  \fn void KoTabDialog::helpButtonPressed();
...
bool KoTabDialog::hasApplyButton() const
{
    return d->ab != 0;
}
...
bool KoTabDialog::hasOkButton() const
{
    return d->ok != 0;
}
...
/*!
  \fn void KoTabDialog::aboutToShow()
  ...
  KoTabDialog() and show(), you should set the
  dialog's state in a slot and connect this signal to it.

  This applies mainly to KoTabDialog objects that are kept around
...
void KoTabDialog::addTab(QWidget * child, const QString &label)
{
...
*/
void KoTabDialog::addTab(QWidget *child, const QIcon& iconset, const QString &label)
{
...
void KoTabDialog::insertTab(QWidget * child, const QString &label, int index)
{
    //d->tw->insertTab(child, label, index);
	d->tw->insertTab(index, child, label);
}
...
/*! \overload
  ...
  label on the tab of \a child.
 */
void KoTabDialog::insertTab(QWidget *child, const QIcon& iconset, const QString &label, int index)
{
...
/*!
  ...
  \sa tabBar()
*/
//void KoTabDialog::setTabBar(QTabBar* tb)
//{
//    if (tb == 0){
//        qWarning("KoTabDialog::setTabBar() called with null QTabBar pointer");
//        return;
//    }
//    
//    d->tw->setTabBar(tb);
...
/*!
  ...
  \sa setTabBar()
*/
//QTabBar* KoTabDialog::tabBar() const
//{
//    return d->tw->tabBar();
//}
...
void KoTabDialog::showPage(QWidget * w)
{
    d->tw->setCurrentWidget(w);
}
...
bool KoTabDialog::isTabEnabled(const char* name) const
{
    if (!name)
	    return false;
    ...
    //        return d->tw->isTabEnabled(d->tw->indexOf(w));
    //    }
    //}

        for (int i = 0; i < d->tw->count(); ++i) {
            if(d->tw->tabText(i)==name)
                return d->tw->isTabEnabled(i);
        }
...
/*!\obsolete
  ...
  KoTabDialog uses QWidget::setEnabled() internally, rather than keeping a
  separate flag.

  Note that even a disabled tab/page may be visible. If the page is
  already visible KoTabDialog will not hide it; if all the pages
  are disabled KoTabDialog will show one of them.
...
void KoTabDialog::setTabEnabled(const char* name, bool enable)
{
    if (!name)
	    return;
    //QObjectList l = this->queryList("QWidget", name, false, true);
    ...
    //}
        for (int i = 0; i < d->tw->count(); ++i) {
            if(d->tw->tabText(i)==name)
                return d->tw->setTabEnabled(i, enable);
        }
}
...
bool KoTabDialog::isTabEnabled(QWidget* w) const
{
    return d->tw->isTabEnabled(d->tw->indexOf(w));
}
...
void KoTabDialog::setTabEnabled(QWidget* w, bool enable)
{
    d->tw->setTabEnabled(d->tw->indexOf(w), enable);
}
...
/*!
  ...
  \sa setCancelButton() setDefaultButton() applyButtonPressed()
*/
void KoTabDialog::setApplyButton(const QString &text)
{
	...
	if (!d->ab) {
	    d->ab = new QPushButton(this);
		d->ab->setText("apply settings");
...
/*!
  ...
  a localizable "Apply".
 */
void KoTabDialog::setApplyButton()
{
    setApplyButton(QObject::tr("Apply"));
}
...
void KoTabDialog::setHelpButton(const QString &text)
{
    if (text.isEmpty()) {
	delete d->hb;
	d->hb = 0;
	...
	if (!d->hb) {
	    d->hb = new QPushButton(this);
		d->hb->setText("give help");
...
/*!
  ...
  a localizable "Help".
 */
void KoTabDialog::setHelpButton()
{
    setHelpButton(QObject::tr("Help"));
}
...
void KoTabDialog::setDefaultButton(const QString &text)
{
    if (text.isEmpty()) {
	delete d->db;
	d->db = 0;
	...
	if (!d->db) {
	    d->db = new QPushButton(this);
		d->db->setText("back to default");
 ...
 */
void KoTabDialog::setDefaultButton()
{
    setDefaultButton(QObject::tr("Defaults"));
}
...
void KoTabDialog::setCancelButton(const QString &text)
{
    if (text.isEmpty()) {
	delete d->cb;
	d->cb = 0;
	...
	if (!d->cb) {
	    d->cb = new QPushButton(this);
		d->cb->setText("cancel dialog");
...
void KoTabDialog::setCancelButton()
{
...
void KoTabDialog::setUpLayout()
{
    ...
    delete d->tll;
    d->tll = new QBoxLayout(QBoxLayout::Down, this);
...
/*!\reimp
*/
void KoTabDialog::resizeEvent(QResizeEvent * e)
{
...
/*!\reimp
*/
void KoTabDialog::paintEvent(QPaintEvent *)
{
}
...
/*!\reimp
*/
void KoTabDialog::showEvent(QShowEvent *e)
{
    if (!e->spontaneous())
        show();
    QDialog::showEvent(e);
...
void KoTabDialog::setOkButton(const QString &text)
{
    if (text.isEmpty()) {
	delete d->ok;
	d->ok = 0;
...
QString KoTabDialog::tabLabel(QWidget * w)
{
    return d->tw->tabText(d->tw->indexOf(w));
}


/*!  \internal
*/
void KoTabDialog::styleChange(QStyle& s)
{
...
QWidget * KoTabDialog::currentPage() const
{
    return d->tw->currentWidget();
}
 ...
 */
void KoTabDialog::changeTab(QWidget *w, const QString &label)
{
 ...
 */
void KoTabDialog::changeTab(QWidget *w, const QIcon& iconset, const QString &label)
{
...
*/
void KoTabDialog::removePage(QWidget * w)
{
    //d->tw->removePage(w);
	d->tw->removeTab(d->tw->indexOf(w));
}
    ...
    KUndo2Command* addShape(KoShape *shape, bool showDialog, KUndo2Command *parent) {
        ...
        if (canvas) {
            if (showDialog) {
                ...
                // show config dialog.
                KoTabDialog *dialog = new KoTabDialog(canvas->canvasWidget());
                dialog->setWindowTitle(QObject::tr("%1 Options").arg(factory->name()));
                ...
                foreach(KoShapeConfigFactoryBase *panelFactory, panels) {
					...
					dialog->addTab(widget, panelFactory->name());
                    pageCount ++;
                }
                foreach(KoShapeConfigWidgetBase* panel, factory->createShapeOptionPanels()) {
                    ...
                    dialog->addTab(panel, title);
                    pageCount ++;
                }

                if (pageCount > 0) {
                    if (pageCount > 1)
                        //dialog->setFaceType(KoTabDialog::Tabbed);
