...
public:
    ...
    void activateToolActions()
    {
        ...
        QMenu *ac = canvas->actionCollection();
        ...
        //    if (ac) {
        //        QAction* action = qobject_cast<QAction*>(ac->action(it.key()));
        //        if (action) {
...
KoToolManagerPrivate::KoToolManagerPrivate(KoToolManager *qq)
    : q(qq),
    canvasData(0),
...
KoToolManagerPrivate::~KoToolManagerPrivate()
{
    qDeleteAll(tools);
}

// helper method.
CanvasData *KoToolManagerPrivate::createCanvasData(KoCanvasController *controller, KoInputDevice device)
{
...
void KoToolManagerPrivate::setup()
{
    if (tools.size() > 0)
        return;
    ...
    foreach(ToolHelper *tool, tools)
        q->connect(tool, SIGNAL(toolActivated(ToolHelper*)), q, SLOT(toolActivated(ToolHelper*)));

    // load pluggable input devices
    KoInputDeviceHandlerRegistry::instance();
}

void KoToolManagerPrivate::switchTool(KoToolBase *tool, bool temporary)
{
    Q_UNUSED(temporary);
    Q_ASSERT(tool);
    if (canvasData == 0)
    ...
    if (newActiveTool) {
        ...
        q->disconnect(canvasData->activeTool, SIGNAL(cursorChanged(const QCursor&)),
                   q, SLOT(updateCursor(const QCursor&)));
        q->disconnect(canvasData->activeTool, SIGNAL(activateTool(const QString &)),
                   q, SLOT(switchToolRequested(const QString &)));
        q->disconnect(canvasData->activeTool, SIGNAL(activateTemporary(const QString &)),
                   q, SLOT(switchToolTemporaryRequested(const QString &)));
        q->disconnect(canvasData->activeTool, SIGNAL(done()), q, SLOT(switchBackRequested()));
        q->disconnect(canvasData->activeTool, SIGNAL(statusTextChanged(const QString &)),
                   q, SIGNAL(changedStatusText(const QString &)));
    }

    canvasData->activeTool = tool;

    q->connect(canvasData->activeTool, SIGNAL(cursorChanged(const QCursor &)),
            q, SLOT(updateCursor(const QCursor &)));
    q->connect(canvasData->activeTool, SIGNAL(activateTool(const QString &)),
            q, SLOT(switchToolRequested(const QString &)));
    q->connect(canvasData->activeTool, SIGNAL(activateTemporary(const QString &)),
            q, SLOT(switchToolTemporaryRequested(const QString &)));
    q->connect(canvasData->activeTool, SIGNAL(done()), q, SLOT(switchBackRequested()));
    q->connect(canvasData->activeTool, SIGNAL(statusTextChanged(const QString &)),
            q, SIGNAL(changedStatusText(const QString &)));
...
void KoToolManagerPrivate::switchTool(const QString &id, bool temporary)
{
    Q_ASSERT(canvasData);
...
void KoToolManagerPrivate::postSwitchTool(bool temporary)
{
#ifndef NDEBUG
    int canvasCount = 1;
    ...
    if (optionWidgetList.empty()) { // no option widget.
        ...
        canvasData->dummyToolLabel->setText(QObject::tr("Active tool: %1").arg(title));
...
void KoToolManagerPrivate::toolActivated(ToolHelper *tool)
{
    Q_ASSERT(tool);
...
void KoToolManagerPrivate::detachCanvas(KoCanvasController *controller)
{
    Q_ASSERT(controller);
...
void KoToolManagerPrivate::attachCanvas(KoCanvasController *controller)
{
    Q_ASSERT(controller);
    ...
    q->connect(connector, SIGNAL(selectionChanged(QList<KoShape*>)), q,
            SLOT(selectionChanged(QList<KoShape*>)));
    q->connect(controller->canvas()->shapeManager()->selection(),
...
void KoToolManagerPrivate::movedFocus(QWidget *from, QWidget *to)
{
    Q_UNUSED(from);
...
void KoToolManagerPrivate::updateCursor(const QCursor &cursor)
{
    Q_ASSERT(canvasData);
...
void KoToolManagerPrivate::switchBackRequested()
{
    Q_ASSERT(canvasData);
    if (!canvasData) return;
...
void KoToolManagerPrivate::selectionChanged(QList<KoShape*> shapes)
{
    QList<QString> types;
...
void KoToolManagerPrivate::currentLayerChanged(const KoShapeLayer *layer)
{
...
void KoToolManagerPrivate::updateToolForProxy()
{
    KoToolProxy *proxy = proxies.value(canvasData->canvas->canvas());
    if(!proxy) return;
...
void KoToolManagerPrivate::switchInputDevice(const KoInputDevice &device)
{
    Q_ASSERT(canvasData);
    if (!canvasData) return;
...
void KoToolManagerPrivate::registerToolProxy(KoToolProxy *proxy, KoCanvasBase *canvas)
{
...
void KoToolManagerPrivate::switchToolByShortcut(QKeyEvent *event)
{
...
void KoToolManagerPrivate::switchToolTemporaryRequested(const QString &id)
{
    switchTool(id, true);
}
...
KoToolManager::KoToolManager()
    : QObject(),
    d(new KoToolManagerPrivate(this))
{
...
void KoToolManager::registerTools(QMenu *ac, KoCanvasController *controller)
{
    Q_ASSERT(controller);
    Q_ASSERT(ac);
    ...
    //    action->setShortcut(th->shortcut().key());
...
KoToolManagerPrivate *KoToolManager::priv()
{
    return d;
}
